...
def process_table(table_block: Dict[str, Any], all_blocks: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    """
    # Define maximum table dimensions
    MAX_ROWS = 1000
    MAX_COLUMNS = 100
    ...
    max_row = max((cell["RowIndex"] for cell in cell_blocks), default=0)
    max_col = max((cell["ColumnIndex"] for cell in cell_blocks), default=0)
    
    if max_row > MAX_ROWS or max_col > MAX_COLUMNS:
        raise ValueError(f"Table dimensions ({max_row}x{max_col}) exceed limits ({MAX_ROWS}x{MAX_COLUMNS})")
...
def extract_text_from_document(bucket: str, key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if file_extension in ["txt", "md", "html", "htm", "csv", "json"]:
            ...
            # Check content length (10MB of text)
            MAX_TEXT_LENGTH = 10 * 1024 * 1024  # 10MB in characters
            if len(content) > MAX_TEXT_LENGTH:
                raise ValueError(f"Text content length {len(content)} exceeds limit of {MAX_TEXT_LENGTH} characters")
